<!-- resources/views/vendor/puja-kits/edit.blade.php -->
@extends('layouts.shopkeeper')

@section('title', 'Edit Puja Kit')
@section('breadcrumb', 'Puja Kits / Edit')
...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Puja Kit</h1>
                <p class="mt-2 text-sm text-gray-700">Update kit information and settings</p>
            </div>
            <a href="{{ route('vendor.puja-kits.show', $pujaKit) }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
        <form method="POST" action="{{ route('vendor.puja-kits.update', $pujaKit) }}" id="kitForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="p-8">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" 
                           ...
                           id="kit_name" 
                           name="kit_name" 
                           value="{{ old('kit_name', $pujaKit->kit_name) }}" 
                           required>
                    @error('kit_name')
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="space-y-2 max-h-60 overflow-y-auto border border-gray-300 rounded-lg p-4">
                                        @php
                                            $selectedPujas = old('pujas', $pujaKit->pujas->pluck('id')->toArray());
                                        @endphp
                                        @foreach($pujas as $puja)
                                            <label class="flex items-center p-2 hover:bg-gray-50 rounded">
                                                <input type="checkbox" 
                                                       name="pujas[]" 
                                                       value="{{ $puja->id }}"
                                                       {{ in_array($puja->id, $selectedPujas) ? 'checked' : '' }}
                                ...
                                <div>
                                    ...
                                    <textarea name="kit_description" 
                                              ...
                                              class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('kit_description', $pujaKit->kit_description) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="space-y-2 max-h-60 overflow-y-auto border border-gray-300 rounded-lg p-4">
                                        @php
                                            $selectedProducts = old('products', $pujaKit->products->pluck('id')->toArray());
                                        @endphp
                                        @foreach($products as $index => $product)
                                            @php
                                                $isSelected = in_array($product->id, $selectedProducts);
                                                $existingPivot = $pujaKit->products->where('id', $product->id)->first();
                                            @endphp
                                            <div class="border border-gray-200 rounded-lg p-3">
                                                <label class="flex items-start">
                                                    ...
                                                    <div class="ml-3 flex-1">
                                                        ...
                                                        <div id="product-options-{{ $index }}" class="mt-3 grid grid-cols-2 gap-3 {{ $isSelected ? '' : 'hidden' }}">
                                                            <div>
                                                                <label class="block text-xs font-medium text-gray-600 mb-1">Quantity</label>
                                                                <input type="number" 
                                                                       name="product_quantities[{{ $index }}]" 
                                                                       value="{{ old('product_quantities.' . $index, $existingPivot ? $existingPivot->pivot->quantity : 1) }}"
                                                                       min="1"
                                                            ...
                                                            <div>
                                                                ...
                                                                <input type="number" 
                                                                       ...
                                                                       value="{{ old('product_prices.' . $index, $existingPivot ? $existingPivot->pivot->price : '') }}"
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Select products and customize quantities and prices</p>
                                    @error('products')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('discount_percentage', $pujaKit->discount_percentage) }}"
                                           min="0"
                                           max="100"
                                           step="0.01"
                                ...
                                <div class="flex items-center">
                                    <input id="is_active" 
                                           ...
                                           type="checkbox" 
                                           value="1"
                                           {{ old('is_active', $pujaKit->is_active) ? 'checked' : '' }}
                                           class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded">
                ...
                <div class="mt-8">
                    ...
                    <div>
                        ...
                        @php
                            $includedItemsText = '';
                            if (old('included_items_input')) {
                                $includedItemsText = old('included_items_input');
                            } elseif ($pujaKit->included_items) {
                                if (is_array($pujaKit->included_items)) {
                                    $includedItemsText = implode("\n", $pujaKit->included_items);
                                } elseif (is_string($pujaKit->included_items)) {
                                    $decoded = json_decode($pujaKit->included_items, true);
                                    $includedItemsText = is_array($decoded) ? implode("\n", $decoded) : '';
                                }
                            }
                        @endphp
                        <textarea name="included_items_input" 
                                  ...
                                  class="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ $includedItemsText }}</textarea>
            ...
            <div class="border-t border-gray-200 px-8 py-6 bg-gray-50 rounded-b-2xl">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('vendor.puja-kits.show', $pujaKit) }}" class="px-6 py-3 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2">
                            Cancel
                        </a>
