...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('admin.categories.edit', $category) }}" class="inline-flex items-center px-4 py-2 bg-purple-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-purple-700">
                    <i data-lucide="edit" class="w-4 h-4 mr-2"></i>
                    Edit Category
                </a>
                <a href="{{ route('admin.categories.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200">
                ...
                <div class="flex-shrink-0">
                    @if($category->image)
                        <img class="h-24 w-24 rounded-xl object-cover border-4 border-white shadow-lg" 
                             ...
                             alt="{{ $category->name }}">
                    @else
                        <div class="h-24 w-24 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center shadow-lg">
                            @if($category->icon)
                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center space-x-3 mb-3">
                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full 
                            ...
                            <i data-lucide="{{ $category->is_active ? 'check-circle' : 'x-circle' }}" class="w-4 h-4 mr-1"></i>
                        ...
                        @foreach($category->children as $child)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex items-center">
                                    @if($child->image)
                                        <img class="h-8 w-8 rounded object-cover mr-3" src="{{ asset('storage/' . $child->image) }}" alt="{{ $child->name }}">
                                    @else
                    ...
                    <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex items-center">
                                    @if($product->featured_image)
                                        <img class="h-8 w-8 rounded object-cover mr-3" src="{{ asset('storage/' . $product->featured_image) }}" alt="{{ $product->name }}">
                                    @else
                                        <div class="h-8 w-8 bg-gray-200 rounded flex items-center justify-center mr-3">
                        ...
                        @if($category->products->count() > 10)
                            <div class="text-center pt-4">
                                <p class="text-sm text-gray-500">And {{ $category->products->count() - 10 }} more products...</p>
                            </div>
                        @endif
                    </div>
                @else
    ...
    <div class="mt-8 flex justify-center space-x-4">
        <a href="{{ route('admin.categories.edit', $category) }}" class="inline-flex items-center px-6 py-3 bg-purple-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2">
            <i data-lucide="edit" class="w-4 h-4 mr-2"></i>
            Edit Category
        </a>
