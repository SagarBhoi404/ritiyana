    ...
    <div class="mb-8">
        <div class="sm:flex sm:items-center sm:justify-between">
            ...
            <div class="mt-4 sm:mt-0">
                <a href="{{ route('admin.pujas.create') }}" class="inline-flex items-center px-4 py-2 bg-purple-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-purple-700">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Puja</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kits</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                @if($puja->image)
                                    <img class="h-10 w-10 rounded object-cover" src="{{ asset('storage/' . $puja->image) }}" alt="{{ $puja->name }}">
                                @else
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500">{{ Str::limit($puja->description, 50) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $puja->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $puja->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.pujas.show', $puja) }}" class="text-blue-600 hover:text-blue-900">
                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                </a>
                                <a href="{{ route('admin.pujas.edit', $puja) }}" class="text-purple-600 hover:text-purple-900">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                </a>
                                @if($puja->kits_count == 0)
                                    <form action="{{ route('admin.pujas.destroy', $puja) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                                        @csrf
                                        @method('DELETE')
